...
export function Promotion() {
  ...
  useEffect(() => {
    if (sourceBatch && targetBatches.length > 0) {
      ...
      if (selectedSourceBatch) {
        // Simple heuristic: Try to match class increment. e.g. "8A" -> "9A", "9B" -> "10B"
  ...
  }, [sourceBatch, targetAy, batches]);

  // Load students for selected source batch
  useEffect(() => {
    if (!sourceBatch) {
      setStudents([]);
      setPromotionStates({});
    ...
    async function loadStudents() {
      ...
      try {
        ...
        // Initialize promotion state for each student
  ...
  const executePromotion = async () => {
    ...
    try {
      ...
      setMessage({
        ...
        text: `Promotion process completed: Successfully updated ${successCount} student(s). ${errorCount > 0 ? `Failed to update ${errorCount} student(s).` : ''}`
      });
      
      // Reload current batch list to reflect changes
      const data = await api.getResources('students', { batch_id: sourceBatch, limit: '1000' });
      const mapped = data.map((s: any) => ({
        id: String(s.id),
        name: s.name,
        ...
        {students.length > 0 && (
          <div className="mt-4 pt-4 border-t border-[var(--b)] flex justify-end">
            <button
              ...
              disabled={executing || targetBatches.length === 0}
