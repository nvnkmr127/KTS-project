...
Schedule::command('etimeoffice:auto-sync', ['--range=today'])
    ...
    ->onFailure(function () {
        ...
        logSchedulerActivity('etimeoffice:auto-sync', 'ETimeOffice Auto-Sync (School Hours)', 'FAILED', ['last_output' => $lastLines]);
    })
...
Schedule::command('etimeoffice:auto-sync', ['--range=today'])
    ...
    ->onFailure(function () {
        ...
        logSchedulerActivity('etimeoffice:auto-sync', 'ETimeOffice Auto-Sync (Off Hours)', 'FAILED', ['last_output' => $lastLines]);
    })
    ->appendOutputTo(storage_path('logs/etimeoffice-sync.log'))
...
Artisan::command('mockdata:seed', function () {
    $this->info('Starting mock data seeding...');
    try {
        $seeder = new \Database\Seeders\MockDataSeeder();
        $seeder->run();
        $this->info('Mock data seeded successfully!');
    } catch (\Exception $e) {
        $this->error('Seeding failed: ' . $e->getMessage());
    }
})->purpose('Seed the database with realistic mock data for testing');

Artisan::command('mockdata:remove', function () {
    $this->info('Removing mock data (restoring database to clean seed state)...');
    try {
        // Run fresh migrations and re-seed the core database seeder
        Artisan::call('migrate:fresh', [
            '--seed' => true,
            '--force' => true
        ]);
        $this->info(Artisan::output());
        $this->info('Mock data removed and database reset successfully!');
    } catch (\Exception $e) {
        $this->error('Failed to remove mock data: ' . $e->getMessage());
    }
})->purpose('Remove mock data and reset the database to fresh seeded state');
