...
class DashboardController extends Controller
{
        ...
        return Timetable::where('user_id', $user->id)
            ...
            ->map(function ($class) {
                return [
                    ...
                    'attendance_taken' => Attendance::where('batch_id', $class->batch_id)
                        ->where('subject_id', $class->subject_id)
                        ->where('time_slot_id', $class->time_slot_id)
    ...
    protected function getMyAttendanceSummary($user): array
    {
        ...
        $allClasses = Timetable::where('user_id', $user->id)->get();
        $classesWithAttendance = 0;
        foreach ($allClasses as $class) {
            $hasAttendance = Attendance::where('batch_id', $class->batch_id)
                ->where('subject_id', $class->subject_id)
                ->where('time_slot_id', $class->time_slot_id)
                ->whereDate('attendance_date', $class->schedule_date)
                ->exists();
        ...
        $attendanceRecords = Attendance::where('faculty_id', $user->id)->get();
    ...
    {
        $attendanceData = Attendance::where('faculty_id', $user->id)
    ...
    protected function getPendingTasks($user): array
    {
        ...
        $todayClassesCollection = Timetable::where('user_id', $user->id)
            ->whereDate('schedule_date', today())
            ->get();

        $todayClasses = $todayClassesCollection->filter(function ($class) {
        ...
        $weeklyClassesCollection = Timetable::where('user_id', $user->id)
            ->whereBetween('schedule_date', [now()->startOfWeek(), now()->endOfWeek()])
            ->where('schedule_date', '<=', today())
            ->get();

        $weeklyPendingAttendance = $weeklyClassesCollection->filter(function ($class) {
            return !Attendance::where('batch_id', $class->batch_id)
                ->where('subject_id', $class->subject_id)
                ->where('time_slot_id', $class->time_slot_id)
                ->whereDate('attendance_date', $class->schedule_date)
        ...
        $overdueClassesCollection = Timetable::where('user_id', $user->id)
            ->where('schedule_date', '<', today())
            ->get();

        $overdueAttendance = $overdueClassesCollection->filter(function ($class) {
            return !Attendance::where('batch_id', $class->batch_id)
                ->where('subject_id', $class->subject_id)
                ->where('time_slot_id', $class->time_slot_id)
                ->whereDate('attendance_date', $class->schedule_date)
    ...
    protected function getAttendanceStatistics($user): array
    {
        ...
        $thisMonthClassesCollection = Timetable::where('user_id', $user->id)
            ->whereMonth('schedule_date', now()->month)
            ->whereYear('schedule_date', now()->year)
            ->get();

        $thisMonthCompleted = $thisMonthClassesCollection->filter(function ($class) {
            return Attendance::where('batch_id', $class->batch_id)
                ->where('subject_id', $class->subject_id)
                ->where('time_slot_id', $class->time_slot_id)
                ->whereDate('attendance_date', $class->schedule_date)
        ...
        for ($i = 3; $i >= 0; $i--) {
            ...
            $weekClassesCollection = Timetable::where('user_id', $user->id)
                ->whereBetween('schedule_date', [$weekStart, $weekEnd])
                ->get();

            $weekCompleted = $weekClassesCollection->filter(function ($class) {
                return Attendance::where('batch_id', $class->batch_id)
                    ->where('subject_id', $class->subject_id)
                    ->where('time_slot_id', $class->time_slot_id)
                    ->whereDate('attendance_date', $class->schedule_date)
    ...
    private function calculateAttendanceStreak($user): int
    {
        ...
        for ($i = 0; $i < 30; $i++) {
            ...
            $dayClassesCollection = Timetable::where('user_id', $user->id)
                ->whereDate('schedule_date', $currentDate)
                ->get();

            $dayCompleted = $dayClassesCollection->filter(function ($class) use ($currentDate) {
                return Attendance::where('batch_id', $class->batch_id)
                    ->where('subject_id', $class->subject_id)
                    ->where('time_slot_id', $class->time_slot_id)
