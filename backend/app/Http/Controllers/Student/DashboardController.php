...
class DashboardController extends Controller
{
    ...
    protected function getTodaysSchedule(Student $student): array
    {
        ...
        return $classes->map(function ($class) use ($student) {
            $attendance = Attendance::where('batch_id', $class->batch_id)
    ...
    protected function getDetailedAttendance($student): array
    {
        ...
        return $attendances->map(function ($attendance) {
            return [
                'date' => $attendance->attendance_date ? $attendance->attendance_date->format('M j, Y') : $attendance->created_at->format('M j, Y'),
                'subject' => $attendance->subject->name ?? 'Unknown',
                'faculty' => $attendance->faculty->name ?? 'Unknown',
                'status' => $attendance->status,
                'time' => $attendance->check_in_time ?: $attendance->created_at->format('H:i A'),
            ];
        })->toArray();
    }
