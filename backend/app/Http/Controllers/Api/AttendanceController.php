...
class AttendanceController extends Controller
{
    ...
    /**
     * Get overall attendance percentages for all students in a batch
     */
    public function getBatchStudentPercentages(Request $request, $batchId): JsonResponse
    {
        try {
            $batch = \App\Models\Batch::with('students')->findOrFail($batchId);
            $user = $request->user();
            ...
            Log::info('getBatchStudentPercentages check:', [
                ...
                'batch_id' => $batchId,
                'cannot_view' => $user ? $user->cannot('view', $batch) : 'no_user',
            ]);
            ...
            // Get all attendance records for this batch grouped by student
            $attendances = Attendance::where('batch_id', $batchId)
            ...
            $studentPercentages = $batch->students->filter(function ($student) {
                ...
                $present = $studentAttendances ? $studentAttendances->whereIn('status', ['present', 'late'])->count() : 0;
                $percentage = $total > 0 ? round(($present / $total) * 100, 1) : 100.0;
            ...
            return ErrorHandler::handleApiException(
                $e,
                'Failed to get batch student percentages',
                'Failed to get student percentages',
                500
            );
        }
    }
